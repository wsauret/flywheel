...
// ---------------------------------------------------------------------------
// Plan parser extracts fulfills annotations from plan markdown
// ---------------------------------------------------------------------------

describe("Plan parser fulfills extraction", () => {
  it("extracts fulfills from HTML comment annotation", () => {
    const content = [
      "## Milestone: Auth",
      "",
      "### Phase 1: Setup auth",
    ...
    expect(phases[0].fulfills).toEqual(["VAL-AUTH-001", "VAL-AUTH-002"]);
  });

  it("extracts fulfills with single assertion ID", () => {
    const content = [
      "### Phase 1: Setup",
      "<!-- fulfills: VAL-SETUP-001 -->",
      "",
      "- [ ] Create project",
    ].join("\n");
    ...
    expect(phases).toHaveLength(1);
    expect(phases[0].fulfills).toEqual(["VAL-SETUP-001"]);
  });

  it("fulfills is undefined when no annotation present", () => {
    const content = [
      "### Phase 1: Setup",
      "",
      "- [ ] Create project",
    ...
    expect(phases).toHaveLength(1);
    expect(phases[0].fulfills).toBeUndefined();
  });

  it("handles multiple phases with different fulfills", () => {
    const content = [
      "## Milestone: Core",
      "",
      "### Phase 1: Auth",
    ...
    expect(phases[0].fulfills).toEqual(["VAL-AUTH-001"]);
    expect(phases[1].fulfills).toEqual(["VAL-API-001", "VAL-API-002"]);
    expect(phases[2].fulfills).toBeUndefined();
  });

  it("handles extra whitespace in fulfills annotation", () => {
    const content = [
      "### Phase 1: Auth",
      "<!--  fulfills:  VAL-AUTH-001 ,  VAL-AUTH-002  -->",
      "",
  ...
  it("fulfills annotation is not included in phase description content", () => {
    const content = [
      "### Phase 1: Auth",
      "<!-- fulfills: VAL-AUTH-001 -->",
      "",
    ...
    expect(phases).toHaveLength(1);
    // The HTML comment should not appear in the description
    expect(phases[0].description).not.toContain("fulfills:");
  });

  it("PlanFileProvider passes fulfills through to PhaseInfo", () => {
    // Import PlanFileProvider here to avoid circular issues
    const { PlanFileProvider } = require("../src/controller/plan-file-provider");
  ...
  it("backward compat: existing plans without fulfills parse unchanged", () => {
    const content = fs.readFileSync(
      path.join(import.meta.dir, "fixtures", "two-phase-plan.md"),
      "utf-8",
    );
    const phases = parsePlan(content);
    ...
    expect(phases[0].fulfills).toBeUndefined();
    expect(phases[1].fulfills).toBeUndefined();
  });
});
