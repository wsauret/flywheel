...
import { buildScaffolding, type ScaffoldingResult, type ScaffoldingPaths } from "../src/workflows/queue/shared/scaffolding";
...
describe("buildScaffolding — strategy map routing", () => {
  it("strategy map handles all StepType values without errors", () => {
    const stepTypes: Array<import("../src/workflows/queue/types").StepType> = [
...
describe("buildScaffolding — field spec integration", () => {
  it("renderHandoffInstruction produces non-empty output for all field spec arrays", () => {
    const { renderHandoffInstruction } = require("../src/workflows/queue/shared/handoff-render");
    const { WORK_STEP_FIELDS } = require("../src/workflows/queue/steps/work/fields");
    const { REVIEW_FIELDS } = require("../src/workflows/queue/steps/review-consolidate/fields");
    const { SHIP_FIELDS } = require("../src/workflows/queue/steps/ship-commit/fields");
    const { SPRINT_FIELDS } = require("../src/workflows/queue/steps/sprint-work/fields");
    const { DEBUG_INVESTIGATE_FIELDS } = require("../src/workflows/queue/steps/debug-investigate/fields");
    const { DEBUG_FIX_FIELDS } = require("../src/workflows/queue/steps/debug-fix/fields");
    const { DEBUG_VERIFY_FIELDS } = require("../src/workflows/queue/steps/debug-verify/fields");
    const { RESEARCH_FIELDS } = require("../src/workflows/queue/steps/research/fields");
    const { SHIP_COMMIT_FIELDS } = require("../src/workflows/queue/steps/ship-commit/fields");
    const { SHIP_LEARNINGS_FIELDS } = require("../src/workflows/queue/steps/ship-learnings/fields");
    const { REVIEW_DISPATCH_FIELDS } = require("../src/workflows/queue/steps/review-dispatch/fields");
    const { REVIEW_CONSOLIDATE_FIELDS } = require("../src/workflows/queue/steps/review-consolidate/fields");
  ...
  it("renderHandoffInstruction requires writing artifacts before the handoff file", () => {
    const { renderHandoffInstruction } = require("../src/workflows/queue/shared/handoff-render");
    const { PLAN_RESEARCH_FIELDS } = require("../src/workflows/queue/steps/plan-research/fields");
