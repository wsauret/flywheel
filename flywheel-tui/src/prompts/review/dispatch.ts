...
### Review Agents (installed as \`fly/*\` agents)

These agents are pre-installed and available via the Task tool. Use \`subagent_type\` to reference each one:

1. **fly/reviewer-architecture** — Layering violations, god objects, circular dependencies, SOLID compliance, pattern breaks.
2. **fly/reviewer-code-quality** — Type safety, naming, duplication, testability. Logic errors, off-by-one, null handling, race conditions, incorrect API usage.
3. **fly/reviewer-patterns** — Design patterns, anti-patterns, naming conventions, code duplication across the diff.
4. **fly/reviewer-performance** — O(n^2) in hot paths, unnecessary allocations, missing pagination, unbounded queries, caching opportunities.
5. **fly/reviewer-data-integrity** — If changes involve data models, migrations, or persistent data: migration safety, constraints, transactions, referential integrity. Skip if not applicable.
