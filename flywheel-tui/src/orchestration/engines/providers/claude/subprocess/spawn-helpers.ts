...
export function buildProcessResult(ctx: ResultContext, exitCode: number): ProcessResult {
...
export function buildErrorResult(ctx: ResultContext, error: unknown): ProcessResult {
  const durationMs = Date.now() - ctx.startTime;
  return {
...
export function createStdoutProcessor(
  ...
  return (text: string) => {
    ...
    if (!wasDetected && completionDetector.hasSeenCompletion) {
      onCompletion();
    }
  };
}
