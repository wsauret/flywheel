...
import type { ToolDefinition, ToolResult, ToolContext, HandoffOperations } from "./types.js";
...
const defaultHandoffOperations: HandoffOperations = {
  writeFile: (path, content) => Bun.write(path, content),
};

export function createHandoffDefinition(options?: { operations?: HandoffOperations }): ToolDefinition {
  const ops = options?.operations ?? defaultHandoffOperations;
  ...
  async function executeHandoff(
    ...
    try {
      await ops.writeFile(context.handoffPath, JSON.stringify(input, null, 2));
...
export const writeHandoffDefinition = createHandoffDefinition();
