...
export async function startChatSession(
  ...
  const engineCmd = engine.buildCommand({ prompt: "", model })
  const spawner = new BunProcessSpawner()
  const sessionId = randomUUID()

  // Budget tracker
  const budgetTracker = createBudgetTracker({ sessionId, baseDir: process.cwd() })
  ...
  spawnResult.result.then(() => {
    ...
    log.warn("chat process error", { error: err instanceof Error ? err.message : String(err) })
    end()
  })
